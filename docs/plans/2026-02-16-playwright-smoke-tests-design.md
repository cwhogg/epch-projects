...
**Date:** 2026-02-16
**Status:** Design
**Scope:** Add Playwright MCP integration for post-deploy smoke testing

## Overview

Add conversational smoke testing via Playwright MCP. After pushing to main and Vercel deploying, ask Claude to smoke test the production URL. Claude reads a flows document and executes each check in the browser, reporting results conversationally.

Pattern borrowed from va-web-app. Key simplification: epch-projects has no app-level auth, so no storage state files needed.

## Files to Create/Modify

| File | Action | Purpose |
|------|--------|---------|
| `.mcp.json` | Create | Playwright MCP server config (headed + headless) |
| `e2e/smoke-test-flows.md` | Create | Natural language smoke test flows |
...
- **`playwright`** — headed mode, visible browser window. Use when debugging a visual issue.
- **`playwright-headless`** — headless mode (default for most smoke tests). Use for quick post-deploy verification.

No npm install needed — `@playwright/mcp` runs via npx on demand.
...
3. Tell Claude:
   > "Smoke test the app at [production-url]. Follow the flows in e2e/smoke-test-flows.md."
4. Claude reads the flows doc, opens a browser via Playwright MCP, and executes each flow
5. Claude reports what worked, what broke, what looked off

**Quick scope:** Say "Run quick smoke tests" — Claude runs only `[QUICK]` tagged flows.
**Full scope:** Say "Run full smoke tests" — Claude runs all flows including interactive journeys.
...
|----------|--------|-----------|
| No storage state files | Omit | App has no auth — publicly accessible |
| Two MCP entries (headed/headless) | Both | Headless for routine checks, headed for debugging visual issues |
| Natural language flows (not Playwright test files) | Conversational | Matches va-web-app pattern, no test runner overhead, Claude adapts to UI changes |
| No CI integration | Manual trigger | Post-deploy verification is manual workflow |
| No npm dependencies | npx only | `@playwright/mcp` runs on demand, no package.json bloat |
